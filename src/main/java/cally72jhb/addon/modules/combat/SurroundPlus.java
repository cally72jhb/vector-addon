...
public class SurroundPlus extends Module {
    ...
    private final Setting<Boolean> stabilize = sgGeneral.add(new BoolSetting.Builder()
            .name("stabilize")
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (centerMode.get() != CenterMode.Never && (centerInAir.get() && !mc.player.isOnGround() || !centerInAir.get())) {
        ...
        if (block != null && (slot >= 0 && slot <= 8 || slot == 45)) {
            ...
            for (Direction direction : Direction.values()) {
                if (direction != Direction.UP && direction != Direction.DOWN) {
                    ...
                    if (doubleHeight) {
                        ...
                        if (packet == null && mc.world.getBlockState(pos).getBlock().getBlastResistance() < 600.0F
                                && canPlace(pos, block.getDefaultState(), mc.world.getBlockState(pos))
                                || packet != null && pos.equals(packet.getPos()) && packet.getState().getBlock().getBlastResistance() < 600.0F
                                && canPlace(pos, block.getDefaultState(), packet.getState())) {
                            positions.add(pos);
                        }
                    }
                }
            ...
            if (!positions.isEmpty()) {
                ...
                for (BlockPos pos : new ArrayList<>(positions)) {
                    if (blocks < blocksPerTick.get() && !places.remove(pos)) {
                        place(pos, slot);
    ...
    private void center() {
        ...
        if (Utils.distance(mc.player.getPos(), pos) >= rangeFromCenter.get()) {
