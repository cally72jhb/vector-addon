...
public class SkeletonESP extends Module {
    ...
    private void on3DRender(Render3DEvent event) {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            if (mc.options.getPerspective() == Perspective.FIRST_PERSON && !freecam.isActive() && mc.player == player) return;

            Color skeletonColor = PlayerUtils.getPlayerColor(player, skeletonColorSetting.get());
            if (distance.get()) skeletonColor = getColorFromDistance(player);

            Vec3d footPos = getEntityRenderPosition(player, delta);
            PlayerEntityModel playerEntityModel = ((PlayerEntityRenderer) (LivingEntityRenderer<?, ?>) mc.getEntityRenderDispatcher().getRenderer(player)).getModel();
            ...
            matrixStack.multiply(new Quaternion(new Vec3f(0, -1, 0), lerpBody + 180, true));
            if (swimming || flying) matrixStack.multiply(new Quaternion(new Vec3f(-1, 0, 0), 90 + headPitch, true));
